...
`harness-init`. Policy rules are not mechanics: confirm with the human which
ones this repo adopts, prune or amend, then treat the survivors as operating
defaults. Promote to blocking lint only when the rule is mechanical, repeated,
and costly if missed.
...
8. **Minimal blocking gates, fix-forward.** Only deterministic checks block
   commits. Review cost is risk-budgeted; cheap fixes beat long waits.
   Non-blocking findings go to the tech-debt tracker.
