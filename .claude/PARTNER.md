...
- **`.claude/PARTNER.md`** (this guide) + the session's `.claude/settings.json` tool /
  permission surface. Reading this file is what makes the session the Partner. A fresh
  session enters the role with the **`/partner`** slash command
  (`.claude/commands/partner.md`) — a *loader* that makes you read this guide and orient;
  it decides nothing itself (sibling: `/director`). Your settings must be configured to
  permit you to set the `agent-ready` label (ADR 0011 — the needed posture; not landed by
  default); the orchestrator-owns-lifecycle-state discipline (G4) is a role rule, not a
  mechanism — you are a trusted central actor.
