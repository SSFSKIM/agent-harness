...
**우리 구현**: `AGENTS.md` 는 63줄이다. 내용은 세 가지뿐 —
...
(progressive disclosure). 그리고 이 "짧게 유지" 자체가 규칙이 아니라 **기계 강제**다:

```python
# plugin/scripts/lint_docs.py — D1 규칙
if n > SIZE_LIMITS["AGENTS.md"]:          # 120줄 초과 시
    _fail(errors, "D1", "AGENTS.md",
          f"{n} lines (max 120).",
          "AGENTS.md is a map, not an encyclopedia: move detail into docs/ and link it.")
```

누군가(사람이든 agent 든) AGENTS.md 를 불리면 커밋 게이트가 빨간불이 되고, 에러
메시지가 "어떻게 고치는지"까지 알려준다.
