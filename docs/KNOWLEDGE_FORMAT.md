...
description: The Knowledge Format v2.0 contract that every knowledge page in this repo follows, making explicit the rules governed by the lint_docs.py D-gate.
---
...
OKF's markdown+frontmatter substrate and adopts its recommended keys. But where
OKF tells consumers *never to reject* (it is a general **exchange** format across
an untrusted boundary), KF runs an **enforced** gate — and **v2.0 flips the old
"permissive on optional" stance**: the load-bearing **navigation** keys
(`type`, `description`, and `phase` on the work-tier anchor) are now *checked
rules*, not suggestions. We are not an exchange format; we are a single actor's
**enforced, fresh working memory**, and a key whose absence silently costs
navigability is a defect, not a free choice. What stays optional is a thin tail
(`tags`, `title`) plus the *validate-if-present* keys (`resource`, `supersedes`,
and `phase` elsewhere) — checked for correctness only when authored. Why we
diverge from OKF here: §7.
...
| `owner` | Who maintains the page (e.g. `harness`, `doc-gardener`, `review-reliability`). |
...
| Key | Requirement |
|---|---|
| `type` | A non-empty `type` — the page's *concept-kind*, queryable independent of directory (vocabulary in §2.3). **Presence** is enforced; the *value* stays free/extensible (OKF's tolerate-unknown is kept for values — a new `type` needs no lint change). |
...
|---|---|---|
| `resource` | scalar | A repo-relative path (preferred) or URL identifying the single primary asset the page documents (e.g. `plugin/scripts/harness_lib.py`). Absent for abstract pages. **D12: if a repo path (not a URL), it must exist.** The precondition for drift detection. |
| `supersedes` | scalar **or list** | Repo-relative `.md` path(s) this page **replaces** — a *declared* pivot edge, the one genuine "design changed" signal. `nav.py` emits a `supersedes` relation to each target (additive to the inferred archived-supersession), surfaced inline in `roadmap`/`map` as `[superseded-by …]` — KF's first declared edge. **D12: each target must resolve.** |
| `phase` | scalar | The initiative + phase, convention `<initiative>/<NN>-<slug>` (e.g. `symphony/04-worker-authority`; a bare `<initiative>` is the umbrella). The **group-by axis for the derived roadmap**: `NN` orders phases within an initiative. **D12: well-formed when present** (`<initiative>` or `<initiative>/<NN>-<slug>`, NN numeric). (Required on `product-spec` — §2.1.) |
...
| `charter` | `CHARTER.md` — top-level intent: mission, core axioms, design philosophy (기획의도) |
...
`type`/`phase` were absent). Promoting a further key remains a governance decision
made on that same evidence.
...
- **v2.0** (current) — **conformance-breaking governance flip**: the navigation
  keys `type` + `description` become required (D11), `phase` becomes required on
  `product-spec` (D11), and `resource`/`supersedes`/`phase` are validated when
  present (D12). The major bump reflects that pages valid under v1.x may now fail.
  Rationale: §7 — we are an enforced working memory, not a permissive exchange.
...
navigation surface OKF leaves permissive — for us an absent `type`/`description`/
`phase` is a defect that silently costs navigability, not a tolerated variation.
Full analysis: [`design-docs/okf-comparison.md`](design-docs/okf-comparison.md).
