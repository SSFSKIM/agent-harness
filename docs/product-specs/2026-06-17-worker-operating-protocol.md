...
| `## Codex Workpad` **단일 Linear 코멘트** = source of truth | **adapt** | 우리 source-of-truth 는 stage 의 **살아있는 repo doc** (research digest / design doc / spec / ExecPlan) — 코멘트보다 풍부하고 보드 쓰기와 안 엮인다. → R2 |
...
- **`WORKFLOW.md` 파일 포팅** · **Linear-코멘트 workpad 메커니즘** · **env-stamp** — triage 의
  reject/이미-보유.
