...
[ADR 0002 — graduated autonomy](../adr/0002-graduated-autonomy.md) 의
...
| `## Codex Workpad` **단일 Linear 코멘트** = source of truth | **adapt** | 우리 narrative 의 authoritative home 은 stage 의 **살아있는 repo doc** (research digest / design doc / spec / ExecPlan) → R2. 단 **board-가시 mirror 를 금지하지는 않는다** — 단일 canonical 진행 코멘트(Symphony workpad 적응)는 [ADR 0002 §2b](../adr/0002-graduated-autonomy.md) slice 2 로 deferred(Director 가 물러나면 사람은 board 를 본다). repo doc=권위, board 코멘트=mirror(경쟁 narrative 아님). |
...
- **board-side canonical 진행 코멘트** — 이 슬라이스는 narrative home 을 repo doc 으로 두는
  데까지만(R2); 단일 board-가시 mirror 코멘트는 [ADR 0002 §2b](../adr/0002-graduated-autonomy.md)
