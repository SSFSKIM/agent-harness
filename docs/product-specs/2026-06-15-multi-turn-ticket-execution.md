...
- **워커는 in-sandbox 를 auto_review 로 self-govern**(슬라이스 3) → mid-turn 승인은 거의 안 옴.
  남는 진짜 "사람 순간"은 **턴-경계 continue/done 판단**이다.
...
- **R2 — 매 턴-종료가 structured worker outcome 을 낳는다.** outcome ∈ {`continuing`, `done`,
  `blocked`(+spawned ticket ids), `needs_human`(question)} — prose 추측이 아니라 신뢰 가능한
  신호. (검증: 각 턴 종료에 outcome 이 분류돼 라우팅됨.)
- **R3 — outcome 별 disposition:**
  - `continuing` → Director 가 **"continue"**(비-taste handle)로 답 → 같은 thread 새 턴. **board
    불변.** (이게 '이제 ExecPlan 할까요?' 케이스.)
  - `done` / `blocked`+children → 워커가 status 전환을 **제안**, Director 가 **집행/검수**. board
    state 는 **여기서만** 바뀐다(워커가 issueCreate 로 만든 자식 + blocked_by 반영).
  - `needs_human` → **taste 만 사람에게 escalate.**
  (검증: continue 는 board 안 건드리고 새 턴; done 만 board 전환; needs_human 은 escalate.)
...
- **R5 — watched vs un-watched.**
  - watched: Director(메인 세션, director-oversight 스킬)가 각 terminal 제안을 검수 + continue 에
    답 + taste escalate.
  - un-watched: `continuing` → **auto-continue**(비-taste 디폴트 = 계속), terminal 제안 → 워커
    신뢰·적용(워커가 그 LLM), `needs_human` → park/안전 기본값 + 사람 async 검토(board/status).
  (검증: un-watched 멀티-턴 런이 워커 신호대로 진행·전환되고 무한 안 돔.)
- **R6 — runaway bound.** 티켓당 max-turns(예: `max_passes` 류) — auto-continue 가 영원히 못
  ...
  stuck 로 보고됨.)
- **R7 — outcome 신호 메커니즘(아래 Design 의 결정).** 워커가 구조화 outcome 을 내는 신뢰 경로 +
  신호 없을 때의 안전 기본값. (검증: 워커가 신호를 내고, 안 내면 디폴트가 안전하게 동작.)
...
drive(ticket):
  ...
  for turn in range(max_turns):         # R6 bound
    result, outcome = run_turn(thread, input)   # outcome = R2 의 구조화 신호
    if outcome.kind == "continuing":
       input = director_continue(ticket, outcome)   # watched: Director 답 / un-watched: "continue"
       continue                          # board 불변, 다음 턴
    else:  # done / blocked / needs_human  → terminal
       director_dispose(ticket, outcome) # watched 검수·집행 / un-watched 워커제안 적용 / taste escalate
       return
  mark_stuck(ticket, "max_turns")        # R6
```

코드(orchestrator)는 위 루프와 라우팅만 소유. **done-ness 판단·board 전환 결정은 워커 outcome +
Director** 가 소유(R4).

### R7 결정 — 워커 outcome 신호

**채택: 필수 `report_outcome` dynamicTool.** 워커에 `linear_graphql` 와 나란히 `report_outcome`
툴을 advertise(`thread/start` `dynamicTools`, D-7 메커니즘 재사용). 워커는 작업 단위를 끝낼 때
이 툴을 호출해 `{status: done|blocked|needs_human, reason, spawned_ticket_ids?, question?}` 를
낸다. 호출이 turn 안에서 오면 그게 terminal 신호; **호출 없이 turn 이 끝나면 디폴트 =
`continuing`**(안전: 계속 가게 두되 R6 bound 가 backstop). 근거: 전용 툴이 prose 파싱보다 신뢰
가능한 구조화 신호 + 기존 tool_executor 경로와 합성 + 워커 설치 skill 이 이미 툴 사용. (대안:
final 메시지 LLM 분류 — 신뢰성 낮아 fallback 으로만.)

**ExecPlan 에서 검증할 것(추측 금지):** Codex 워커가 "끝낼 때 이 툴을 불러라" 프롬프트에 얼마나
신뢰성 있게 따르는지 — 안 따르면 "no-signal ⇒ continue, bounded" 디폴트가 안전망. (multi-turn
continuation 자체는 이미 live 검증됨.)

### 구성요소 / 파일

- **`director/run.py`** — `run_ticket`(한 턴)을 **multi-turn `drive`**(thread 유지, outcome 까지
  반복)로. 한 턴 단위는 내부 helper 로 남김.
- **`director/orchestrator.py`** — `reconcile` 의 `completed → Done` 코드 매핑 **제거**; dispatch 가
  `drive` 를 호출하고 그 terminal outcome 을 board 로 집행(Director 결정/워커 제안 경유). DAG/
  concurrency/wave 루프는 유지.
- **`report_outcome` 툴** — `director/worker/tools.py`(또는 신규)에 spec + executor; outcome 을
  큐/Director 로 라우팅.
- **continuation 판단** — watched: `.claude/skills/director-oversight/`(continue/done/escalate
  가이드 확장; 슬라이스 2 스킬이 여기서 본래 목적을 되찾음); un-watched: auto-continue 코드 경로.
- **status 표면** — `director/status.py` 에 턴-수/continuation 반영(R8).

### 에러 / 경계 케이스

- 워커가 `report_outcome` 안 부르고 turn 종료 → `continuing` 디폴트, R6 bound 가 무한 방지.
...
- **report_outcome 스키마의 과한 일반화.** 4개 kind 로 시작(YAGNI).

## Acceptance criteria

- 한 티켓이 ≥2 턴에 걸쳐 같은 thread 로 진행되고, 중간 `continuing` 에서 board 가 안 바뀌며,
  terminal `done` 에서만 board 전환(R1/R3/R4).
- 워커가 `report_outcome(done)` 부르면 terminal 처리; 안 부르고 끝나면 `continuing` 디폴트로
  다음 턴; max-turns 초과면 stuck(R2/R6/R7).
- `blocked`+spawned children 가 보고되면 board 가 Blocked + 자식 티켓 픽업(R3).
- orchestrator 에 turn-status→board-state 직접 매핑이 없음(R4 — diff 로 확인).
- un-watched 멀티-턴 런이 워커 신호대로 진행·전환, 무한 안 돔(R5/R6).
- **Live wire-pin:** 실제 codex 워커가 2+ 턴짜리 작업을 `report_outcome(done)` 로 마치는 1회.
...
- **D-44 outcome 신호 = 필수 `report_outcome` 툴, fallback "no-signal ⇒ continue bounded".**
  전용 툴이 신뢰 가능한 구조화 신호; 신뢰성은 ExecPlan 에서 검증.

## Open Questions

- `report_outcome` 신뢰성(워커가 끝낼 때 실제로 부르나) — ExecPlan live 검증; 안 되면 no-signal⇒
  continue 디폴트 + (옵션) final-message LLM 분류.
