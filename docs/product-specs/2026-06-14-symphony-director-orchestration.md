...
- **Phase 3 — 티켓 DAG + dev-stage taxonomy.** `blocked_by`, 에이전트 self-created 티켓,
  ...
  매핑(각 type 이 docs/product-specs·docs/exec-plans 등 하네스 doc 를 산출). 분해(사람,
  2026-06-14): **3a — DAG 메커닉**(자기 spec:
  [dag-aware-orchestration](2026-06-14-dag-aware-orchestration.md), blocked_by + 연속
  wave 루프 + 워커-생성 픽업) 먼저, **3b — dev-stage taxonomy**(자기 spec:
  [dev-stage-taxonomy](2026-06-14-dev-stage-taxonomy.md), type→methodology+산출물+typed
  분해, full 5-stage) 다음.
- **Phase 4 — 자율 Director.** loop/scheduled oversee, board 리포팅, taste-vs-handle
  escalation 정책, PR 머지 관리. 분해(사람, 2026-06-14): linked 슬라이스로 쪼갬 — **첫
  슬라이스 = worker authority guardrail**(자기 spec:
  [worker-authority-guardrail](2026-06-14-worker-authority-guardrail.md), un-watched
  의 하드 선행조건, tracker line 49) 먼저, 이어 **taste-vs-handle escalation policy**(자기 spec:
  [director-orchestration-visibility](2026-06-15-director-orchestration-visibility.md) — 가시성
  우선으로 재슬라이싱: 오케스트레이션 상태를 atomic 스냅샷으로 영속화 + 인라인 Director 가
  read-API/스킬로 판단, escalation 은 헤드리스 judge 가 아니라 메인 세션 D-5) → **un-watched
  자율**(D-38: 워커가 Codex 자체 approval 게이트로 self-govern — `--autonomous` preset, ExecPlan
  `worker-autonomy-config`, 보안 T11; 헤드리스 Director 기각) → **multi-turn 티켓 실행**(자기 spec:
  [multi-turn-ticket-execution](2026-06-15-multi-turn-ticket-execution.md) — D-39+: 티켓=여러 턴,
  턴 종료≠완료, 워커가 status 제안/Director 집행, 코드는 done-ness 판단 0; RV2 주력 케이스 구현,
  Phase 2 reconcile 재설계) → **PR-merge 관리**(자기 spec:
  [worker-qa-and-serialized-pr-merge](2026-06-16-worker-qa-and-serialized-pr-merge.md) +
  ExecPlan `activate-serialized-merge-pipeline`/`merge-reenqueue-loop`) → **board reporting**(자기
  spec: [director-board-reporting](2026-06-16-director-board-reporting.md)).
  **상태(2026-06-16): Phase 4 로드맵 슬라이스 전부 shipped** — completed/ ExecPlan 들 참조. 남은
  Phase-4-인접 작업은 워커 secret 하드닝 트랙(fs-wide cred read + egress → container plan + GCP
  vault-proxy plan; tracker)뿐 — 별도 future effort.
...
- **D-38 un-watched 자율 = Codex approval 설정, 헤드리스 Director 아님.** Phase 4 "loop/scheduled
  oversee" 의 핵심 문제("사람이 떠나면 throughput 0")는 워커의 보수적 `untrusted` 기본값의
  산물이다. Codex 가 per-action approval 을 자체 소유한다(`thread/start` 의 `approvalPolicy`/
  `sandbox`, + `approvals_reviewer=auto_review` 라는 fail-closed 리뷰어 에이전트). 따라서 Director
  는 per-action 승인자가 **아니다** — 그 역할은 orchestration + T10(host 키 Linear 쓰기) + 드문
  진짜 질문뿐. 자율 실행 모델은 이미 있다(orchestrator `run_until_drained` 가 un-watched 드라이버).
  un-watched 의 lever = 워커 posture 를 `on-request + auto_review + workspace-write + full network`
  로 튜닝(`--autonomous` preset, default off). 헤드리스 `claude -p` Director(폐기된 imprint 패턴)는
  auto_review 를 더 나쁘게 재구현하는 것이라 기각. 빌드: ExecPlan `worker-autonomy-config`,
  보안 SECURITY.md T11. (사람 결정, 2026-06-15.)

## 열린 질문 (Open Questions)

- ~~Director invocation 모델~~ → **해소: main 세션**(D-5).
- ~~`LINEAR_API_KEY` secret 취급~~ → **해소: `.env`(gitignored)**(D-6).
- ~~taste-vs-handle 경계의 구체적 정책(무엇을 사람에게 올리나)~~ → **해소(Phase 4 둘째 슬라이스):**
  [director-orchestration-visibility](2026-06-15-director-orchestration-visibility.md) R7 —
  인라인 Director 가 오케스트레이션 맥락 위에서 handle-inline vs. escalate 를 가르고, 불확실하면
  escalate(fail-safe). 판단은 별도 헤드리스 judge 가 아니라 메인 세션(D-5/그 spec D-30).
