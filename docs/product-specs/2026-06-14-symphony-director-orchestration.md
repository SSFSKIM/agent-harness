...
- **Phase 2 — Orchestrator.** poll 루프, dispatch, per-ticket workspace, concurrency·
  retry·reconciliation(Symphony state machine), 다중 워커.
...
- Director invocation 모델: 사람이 띄우는 main 세션 vs `/loop` self-paced vs scheduled.
  (Phase 4 에서 확정; Phase 1 은 최소 responder.)
- taste-vs-handle 경계의 구체적 정책(무엇을 사람에게 올리나). (Phase 4.)
- `director/` 배치 확정: host app-code 로 top-level 이 맞는지, 게이트/테스트 연결 방식.
  (Phase 1 ExecPlan.)
- Linear 워크스페이스/권한·`LINEAR_API_KEY` 운용과 secret 취급(SECURITY.md 연계).
